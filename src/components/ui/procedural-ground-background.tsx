 ...
 * A WebGL 2D background featuring topographic lines tuned to red hues.
 */
const ProceduralGroundBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        vec3 baseColor = vec3(0.06, 0.02, 0.02); // very dark red / near black
        vec3 accentColor = vec3(0.6, 0.08, 0.08); // deep red
        vec3 neonColor = vec3(1.0, 0.18, 0.18);   // bright neon red
  ...
  if (webglSupported === false) {
    return <AnimatedBackground color={'200,16,46'} />;
  }
  ...
  return (
    <div className="fixed inset-0 w-full h-full -z-1 pointer-events-none">
      <canvas
        ...
        style={{ filter: 'contrast(1.05) brightness(0.95)', zIndex: -1 }}
      />
    </div>
  );
};
